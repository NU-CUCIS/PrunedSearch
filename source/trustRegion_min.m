...
function [bestOpt, bestOdf] = trustRegion_min(fhandle)

%This function calls global optimization tool fmincon to perform optimization
%   Since the fmincon function requires a starting point (odf0), in
...
%
%The function fmincon initially deals with minimization, in order to
...
%   for example, fmincon_min(@proper1Opt)
...
for j = 1: 76 % for each variable
    ...
    for i = 1:length(searchRangeWide)
        ...
        % search with fmincon
        [odf,opt] = fmincon(fhandle,odf0,[],[],constraint,1,zeros(1,76),ub);
...
save('./OptimizeResults/Property3/prob3_TrustRegion.mat','bestODF','bestOPT');
